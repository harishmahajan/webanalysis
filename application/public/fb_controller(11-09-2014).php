...
*/
    ...
    private function utc_datetime_to_pst_date($uts_datetime,$deduct_days = 0)
    {
        ...
        // deduct 7 hours to convert it to PST
        $pst_time = ( $utc_time - ( 7 * 60 * 60 ) );
    ...
    private function loadFbData($oCon, $from, $to)
    {
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'https://graph.facebook.com/'.$oCon->ExternalConnectionID.'/insights/?'.http_build_query($get_data));
        ...
        foreach($oResult->data as $oData)
        {
            if($oData->name == 'page_fans')
            {
                ...
                foreach($oData->values as $oVal)
                {
                    ...
                    $data[$this->utc_datetime_to_pst_date($oVal->end_time)] = $oVal->value;
                }
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'https://graph.facebook.com/'.$oCon->ExternalConnectionID.'/posts/?'.http_build_query($get_data));
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);  //to suppress the curl output
        $oResult = curl_exec($ch);
        curl_close ($ch);
        $oResult = json_decode($oResult);
        ...
        foreach($oResult->data as $oData)
        {
            ...
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, 'https://graph.facebook.com/'.$oData->id.'/insights/?'.http_build_query($get_data));
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);  //to suppress the curl output
...
//            print_r($oResult2->data);
    ...
    {
        return \Utility\Db::query("select ConnectionID, AccountID, ExternalConnectionID, ExternalConnectionData from Connection where Type = 'facebook' and ExternalConnectionData is not null",array(),true);
    }
