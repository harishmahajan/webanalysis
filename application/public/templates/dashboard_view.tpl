...
<nav class="navbar navbar-default" role="navigation" style="margin-top:15px; margin-bottom: 15px;">
    <div class="container-fluid">
        <a class="btn btn-info navbar-btn navbar-right btnPadding" href="/account/add/{$conn.AccountID}" id="btnAddAC">Create New Account</a>
    </div>
</nav>
...
<nav class="navbar navbar-default" role="navigation" style="margin-top:15px; margin-bottom: 15px;">
    ...
    <div class="container-fluid">
        ...
        <a class="btn btn-default navbar-btn navbar-right btnPadding" href="/reports/view/{$conn.AccountID}/{$conn.ConnectionID}">View Full Report</a>
    </div>
</nav>
...
<div class="row">
    <div class="col-md-6">
            ...
            <table class="table table-bordered">
                ...
                <tr>
                    <td class="text-center">
                        <div class="row">
                            <div class="col-sm-6">
                                <h2>{math equation="x + y + z + s" x=$conn.favorites|number_format y=$conn.mentions|number_format z=$conn.retweets|number_format s=$conn.replies|number_format}</h2>
                            </div>
                            <div class="col-sm-6">
                                <h2>{$conn.User->FollowersCount|number_format}</h2>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-sm-6">
                ...
                <tr>
                    <td class="text-center">
                        <div class="row">
                            <div class="col-sm-3">
                                <h4>{$conn.replies|number_format}</h4>
                            </div>
                            <div class="col-sm-3">
                                <h4>{$conn.retweets|number_format}</h4>
                            </div>
                            <div class="col-sm-3">
                                <h4>{$conn.mentions|number_format}</h4>
                            </div>
                            <div class="col-sm-3">
                                <h4>{$conn.favorites|number_format}</h4>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-sm-3">
        ...
        <div class="panel panel-default">
            <div class="panel-heading">Summary</div>
            <div class="panel-body">
                During the last 7 days, {$conn.User->Name} (@{$conn.User->ScreenName}) gained <span class="highlite">{$conn.NewFollowers->new}</span> new followers on Twitter
                    (<span class="highlite">{math equation="x * 100 / y" format="%.2f" x=$conn.NewFollowers-new y=$conn.User->FollowersCount}% growth</span>) to reach a total fan base of
                    <span class="highlite">{$conn.User->FollowersCount} users</span>.

                <span style="color:red">This month, each published Tweet yielded an average of {math equation="x / y " format="%.2f" x=$conn.Potential_Impression->PotentialImpression y=$conn.tweets  } potential impressions and, as a whole.</span>
