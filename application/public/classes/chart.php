...
class Chart
{
    ...
    public function Demographics()
    {
        $aoData = \Utility\Db::query("select
            ...
            from
                DataFb f
            , DataFb_PageFansGenderAge p where p.DataFbID = f.DataFbID
            and
               f.ConnectionID = ? and
    ...
    public function Mentions($iUserId)
    {
        ...
        $aoData = \Utility\Db::query("select date_format(CreatedAt,'%m/%d') day,sum(IsMention)+sum(IsReplie) as mentions from webanalysis.`DataTwMentation` where ConnectionID=? and CreatedAt between ? and ? group by DATE_FORMAT(CreatedAt, '%Y/%m/%d')",array($this->iConnection,$sFrom,$To),true);
    ...
    public function InternalFanSources()
    {
        $aoData = \Utility\Db::query("select
                ...
                sum(c.Value) val
            from
                DataFb f, DataFb_PageFansByLikeSource c where c.DataFbID = f.DataFbID
            and
                f.ConnectionID = ? and
                f.`Date` between ? and ?
            group by
    ...
    {        
        ...
        if($MAX<=1)
        {
            $aChartData['label'][] = $aoData->day;
            if(!empty($aoData->free))
                $aChartData['line1'][] = (int)$aoData->free;
            else
                $aChartData['line1'][] =0;

            if(!empty($aoData1->paid))
                $aChartData['line2'][] = (int)$aoData1->paid;
            else
                $aChartData['line2'][] =0;
        }
        else
    ...
    public function ActiveDemographics()
    {
        ...
        if($this->aQuaryArray[2] != 'img')
            ...
            return $oChart->show('BarChart',$this->aQuaryArray[2],array('hAxis'=>array('textPosition'=> 'none')));
