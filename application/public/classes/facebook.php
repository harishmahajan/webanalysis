...
class Facebook{
    ...
    public static function connectioninsert($FBAccountId, $FBpageID,$FBacctok)
    {
        //return true;
        if( \Utility\Db::query("select 1 from webanalysis.Connection where ExternalConnectionID=?",array($FBpageID)))
        {
            echo "false";
        }
        else
        {
            \Utility\Db::query("INSERT INTO `Connection` (`AccountID`, `ExternalConnectionID`, `ExternalConnectionData`,`Type`) VALUES (?, ?, ?,'facebook')",array($FBAccountId, $FBpageID,json_encode($FBacctok)));
            $this->iConnectionId = \Utility\Db::last_insert_id();
            echo "true";
        }
    ...
    public function update_page_fans($aData)
    {
        ...
        foreach($aData as $sDate => $iValue)
        {
            ...
            //new code
            $new_date = date("Y-m-d",strtotime($sDate));
            $iDataFbID = $this->getDataFbId($new_date);
            $this->_update_DataFb($iDataFbID,array('PageFans'=>$iValue));
        }
    }
    ...
    {
        if(empty($aData)) return;
        file_put_contents('/var/www/application/public/log.txt',"in update_page_storytellers_by_age_gender ".print_r($aData,true).PHP_EOL, FILE_APPEND);
...
\Utility\Db::query("delete from webanalysis.DataFb_Posts where Post_link is null and Post_story_adds=0 and Post_impressions_viral_unique=0 and Post_engaged_users =0 and Post_like is null and Post_comment is null and Post_share is null");
    ...
    {        
        ...
        $this->_update_table_DataFb_PageFansByLikeSource('DataFb_PageFansByLikeSource','Source','Value',$aData);
    }
    ...
    private function _update_table_DataFb_PageFansByLikeSource($sTable, $sNameField, $sValueField, $aData)
    {    
