...
class Reports extends \Webpage
{
    ...
    public function ReadFilePDF()
    {
        //$var=file_put_contents('/var/www/application/public/pdf.txt');
        if($_SESSION['PDFVar']=="1")
        {
            echo $_SESSION['PDFVar'];
            $_SESSION['PDFVar']="0";
            exit();
        }
        else
    ...
    public function view(){
        ...
        if($aAccounts = \Account::getClientAccounts(\Utility\Client::getClientId()))
        {
            ...
            {  
                ...
                if(empty($this->aTplVars['aoConnections']))
                {
                    \Utility\Url::phpRedirect('/account/connections/'.$this->aQuaryArray[2]);
                }
                else
                {
                    ...
                    { 
                        \Utility\Url::phpRedirect('/reports/view/'.$this->aQuaryArray[2].'/'.$this->aTplVars['aoConnections'][0]->ConnectionID);                 
                        /* old
                ...
                if($oConnection->Type == 'facebook')                {
                    ...
                    //new code or Recent Community Growth
                    ...
                    if($interval->format('%a')<90)
                        ...
                        $oChart = new \Chart(array('chart','RecentCommunityGrowth',($this->isPdf?'img':'html'),$oConnection->ConnectionID,date("Y-m-d",$from_time),date("Y-m-d",$to_time)));
                ...
                {   
                    ...
                    $aReportTplVars = array(
                        ...
                        // get new followers
                        'NewFollowers' => \Twitter_data::getNewFollowers($this->aQuaryArray[3],$oUserData->DataTwUserID,date("Y-m-d",$from_time),date("Y-m-d",$to_time)),

                        'favorites' => \Twitter_data::getFavorites($this->aQuaryArray[3],$oUserData->DataTwUserID,date("Y-m-d",$from_time),date("Y-m-d",$to_time)),
                        'mentions' => \Twitter_data::getMentions($this->aQuaryArray[3],$oUserData->DataTwUserID,date("Y-m-d",$from_time),date("Y-m-d",$to_time)),
                        'retweets' => \Twitter_data::getRetweets($this->aQuaryArray[3],$oUserData->DataTwUserID,date("Y-m-d",$from_time),date("Y-m-d",$to_time)),
                        'replies' => \Twitter_data::getReplies($this->aQuaryArray[3],$oUserData->DataTwUserID,date("Y-m-d",$from_time),date("Y-m-d",$to_time)),

                        'tweets' => \Twitter_data::getTweets($this->aQuaryArray[3],$oUserData->DataTwUserID,date("Y-m-d",$from_time),date("Y-m-d",$to_time)),
                        'dateRange'=>\Twitter_data::getRange(date("Y-m-d",$from_time),date("Y-m-d",$to_time)),
                        'recent_tweets' =>\Twitter_data::getRecentTweets($this->aQuaryArray[3],date("Y-m-d",$from_time),date("Y-m-d",$to_time),($this->isPdf)?8:5),          
                        'most_active_fans' => \Twitter_data::getMostActiveFans($this->aQuaryArray[3],date("Y-m-d",$from_time),date("Y-m-d",$to_time)),
                        'top_locations' => \Twitter_data::getTopLocations($this->aQuaryArray[3],date("Y-m-d",$from_time),date("Y-m-d",$to_time),($this->isPdf)?10:5),
                        'Potential_Reach'=>\Twitter_data::getPotentianReach($this->aQuaryArray[3],date("Y-m-d",$from_time),date("Y-m-d",$to_time)),
                        'Unique_People'=>\Twitter_data::getUniquePeople($this->aQuaryArray[3],date("Y-m-d",$from_time),date("Y-m-d",$to_time)),
                        'Potential_Impression'=>\Twitter_data::getPotentialImpression($this->aQuaryArray[3],date("Y-m-d",$from_time),date("Y-m-d",$to_time)),
                        'Charts' => $aCharts
                    ); 
                    $this->aTplVars['report'] = \Utility\SmartyTemplate::fetchTemplate(
                ...
                {
                    $errors=array();
                    $errors= \Account::GetErrors($this->aQuaryArray[2]);
                    
                    if(!empty($errors["ConErr"]) || !empty($errors["FbNullTok"]))
                    {
                        $errors['AccID']=$this->aQuaryArray[2];
                        $this->addSub('errorshow.tpl','popups',$errors);
                    }
