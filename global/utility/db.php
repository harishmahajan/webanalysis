...
class Db
{
    ...
    private static function getDbHandle($_dbalet=false){
        ...
        if(!isset(self::$_dbHandle[$dbindex])){
    ...
    public static function CronQuery($bulkQuery, $aletrnativeDB=false)
    {   
        ...
        //$CronBD=new PDO("mysql:host=localhost;dbname=webanalysis", 'webanal', '34&TwgrweDFwe4%');        
        //$CronBD=new PDO("mysql:host=localhost;dbname=webanalysis", 'root', 'jack123');        
        $CronBD=new PDO("mysql:host=us-cdbr-iron-east-03.cleardb.net;dbname=ad_5190e5be64d2882", 'b6e33f46feda20', '8a9e8cc5');        
        $CronBD->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
        $CronBD->setAttribute(PDO::ATTR_EMULATE_PREPARES, 1);
        $CronBD->exec($bulkQuery);

       /* $Cronsth = $CronBD->prepare($bulkQuery);
        $Cronsth->execute();*/

       /* $db = self::getDbHandle($aletrnativeDB);
        $sth = $db->prepare($bulkQuery);
        $result = $sth->execute();*/
      
        //$db->exec($bulkQuerys);
/*        $stmt = $db->prepare($bulkQuerys);
        $stmt->execute();*/
    }
